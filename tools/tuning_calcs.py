...
def calc_tun_quality(tun_value):
  ...
  tun_qual = [[[0 for item in subl] for subl in tun_value[0]] for cat in range(len(tun_value))]

  for cat in [0,1,2,3]:
    for layer in range(len(tun_value[cat])):
      for map in range(len(tun_value[cat][layer])):
        if tun_qual[cat][layer][map] < tun_value[cat][layer][map]:
          tun_qual[cat][layer][map] = tun_value[cat][layer][map]

  tun_qual = [[i for i in temp if i !=0] for temp in tun_qual]
  return tun_qual

from keras import Model
def calc_tun_activ(merge_data,model,categories):
